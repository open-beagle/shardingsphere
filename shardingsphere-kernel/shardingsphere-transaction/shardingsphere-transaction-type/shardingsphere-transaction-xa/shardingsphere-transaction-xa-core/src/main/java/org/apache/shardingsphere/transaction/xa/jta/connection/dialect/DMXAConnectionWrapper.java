...
/**
 * XA connection wrapper for DM. by wuwanli
 */
public final class DMXAConnectionWrapper implements XAConnectionWrapper {
    ...
    private Class<Connection> getJDBCConnectionClass() {
        return (Class<Connection>) Class.forName("dm.jdbc.driver.DmdbConnection");
    }

    @SneakyThrows(ReflectiveOperationException.class)
    private Constructor<?> getXAConnectionConstructor() {
        return Class.forName("dm.jdbc.driver.DmdbXAConnection").getConstructor(Class.forName("dm.jdbc.driver.DmdbConnection"));
    }
